...
GLuint	helicopter;				// object display list
GLuint	Blade;
GLuint  Obj;
int		ModeOn;
...
Animate( int option )
{
	...
	switch (option) {
	case 1:
		glRotatef(Time * 360, 0., 1., 0.);// for front
		break;
	case 2:
		glRotatef(3 * Time * 360, 1., 0., 0.);// for rear
		break;
	}
...
Display( )
{
	...
	{
		//Reset();
		gluLookAt(-0.4, 1.8, -4.9, -0.4, 1.8, -30., 0., 1., 0.);
	}
	else if (ModeOn == 0) {
		//Reset();
...
InitMenus( )
{
	...
	int modemenu = glutCreateMenu(DoMode);
...
InitLists( )
{
	...
	for (int i = 0; i < 10; i++)
	{
		glVertex3f(-5 + i, 0.8, -19);
		glVertex3f(-5 + i, 0.8, -10);
	...
	for (int i = 0; i < 10; i++)
	{
		glVertex3f(-5 + i, 5.8, -19);
		glVertex3f(-5 + i, 5.8, -10);
	...
	for (int i = 0; i < 5; i++)
	{
		glVertex3f(-5, 0.8 + i, -19);
		glVertex3f(-5,0.8 + i, -10);
