...
const int MS_IN_THE_ANIMATION_CYCLE = 4000;
...
Animate( )
{
	...
	// for Display( ) to find:
		...
		// milliseconds in the animation loop
...
Display( )
{
	...
	glPushMatrix();
		...
		glTranslatef(4., 4., 0.);
		SetPointLight(GL_LIGHT0, 2., 2., 0., 1., 1., 1.);
		if (Light0On) {
	...
	glPushMatrix();
		...
		glTranslatef(-2., 0., 0.);
		SetSpotLight(GL_LIGHT1, 0., 0., 0., 20., 20., 0., 1., 0., 0.);
		if (Light1On) {
			glDisable(GL_LIGHTING);
			glEnable(GL_LIGHT1);
		}else{
	...
	glPushMatrix();
		glShadeModel(GL_FLAT);
		SetMaterial(1., 0., 0., 128.);
		glTranslatef(0., 0., 5.*cos(2*Time*M_PI));
		glCallList(BoxList);
...
InitLists( )
{
	...
	GLUquadric* quad_x;
	spot_x = glGenLists(1);
	glNewList(spot_x, GL_COMPILE);
	glPushMatrix();
	quad_x = gluNewQuadric();
	gluSphere(quad_x, 0.5, 20, 20);
	glPopMatrix();
	glEndList();*/
