...
Sphere(float radius, int slices, int stacks)
{
	...
	for (int ilat = 0; ilat < NumLats; ilat++)
	{
											...
											// ilng=NumLngs-1, lng=+M_PI are the same meridian
		{
			...
			p->nz = z;
			if (Distort) {
				p->s = (lng + M_PI) / (2. * M_PI) + cos(2. * (lat + M_PI / 2. + Time)) * 0.17;
				p->t = (lat + M_PI / 2.) / M_PI;
			}
			else {
				p->s = (lng + M_PI) / (2. * M_PI);
...
//#define MS_IN_THE_ANIMATION_CYCLE 6000
void
Animate( )
{
	...
	Time = M_PI * (float)ms / (float)MS_IN_THE_ANIMATION_CYCLE;        // [ 0., 1. )
...
InitMenus( )
{
	...
	int mapemenu = glutCreateMenu(DoMapeMenu);
	glutAddMenuEntry("Off", 0);
	glutAddMenuEntry("On", 1);
	...
	glutAddSubMenu(   "Texture",  mapemenu);
