...
int     Mapping;              //!=0 means enalbe texture
...
char* bmp[13] = { "8k_sun.bmp", "2k_mercury.bmp", "2k_venus.bmp", "2k_earth_daymap.bmp", "2k_mars.bmp", "jupiter.bmp", "2k_saturn.bmp", "2k_uranus.bmp", "2k_neptune.bmp", "pluto.bmp", "pinksky.bmp", "saturnring.bmp" , "moon.bmp" };
GLuint tex[13];
...
void Textplanet(char* bmp[13], GLuint tex[13]) {
	for (int i = 0; i < 13; i++) {
...
void saturnring(float radius1, float radius2) {
...
Display()
{
	...
	int sun_rotation_period = 25379;
	glRotatef(360 * (float(ms % sun_rotation_period) / sun_rotation_period), 0., 1., 0.);
	Sphere(4.0, 50, 50); 
	glDisable(GL_TEXTURE_2D);
	...
	//mercury
	drawcircle(0.39 + 5.0); 
	int mercury_orbit_period = int(o_p(5.39));
	...
	int mercury_rotation_period = 58646; 
	glRotatef(360 * (float(ms % mercury_rotation_period) / mercury_rotation_period), 0., 1., 0.);
	glShadeModel(GL_SMOOTH);
	SetMaterial(1.0, 1.0, 1.0, 20.0);
	...
	glEnable(GL_TEXTURE_2D);
	glPushMatrix();
	int venus_orbit_period = int(o_p(0.72 + 7.0));
	glRotatef(360 * (float(ms % venus_orbit_period) / venus_orbit_period), 0., 1., 0.); 
	glTranslatef(0.72 + 7.0, 0., 0.);
	glShadeModel(GL_SMOOTH);
	SetMaterial(1.0, 1.0, 1.0, 20.0);
	int venus_rotatoin_period = 243018; 
	...
	glPushMatrix();
	int earth_orbit_period = int(o_p(1. + 8.));
	glRotatef(360 * (float(ms % earth_orbit_period) / earth_orbit_period), 0., 1., 0.); 
	...
	glRotatef(-23, 1, 0, 0); 
	int earth_rotation_period = 997; 
	glRotatef(360 * (float(ms % earth_rotation_period) / earth_rotation_period), 0., 1., 0.);
	Sphere(1.0, 100, 100);  
	
	drawcircle(1.2); 
	glEnable(GL_TEXTURE_2D);
	glPushMatrix();
	int moon_orbit_period = int(o_p(1.2));
	int moon_rotation_period = 27321;
	glRotatef(360 * (float(ms % moon_orbit_period) / moon_orbit_period), 0., 1., 0.);
	glTranslatef(1.2, 0., 0.);
	glRotatef(360 * (float(ms % moon_rotation_period) / moon_rotation_period), 0., 1., 0.);
	...
	glEnable(GL_TEXTURE_2D);
	glPushMatrix();
	int mars_orbit_period = int(o_p(1.52 + 9.));
	...
	glEnable(GL_TEXTURE_2D);
	glPushMatrix();
	int jupiter_orbit_period = int(o_p(5.20 + 10));
	glRotatef(360 * (float(ms % jupiter_orbit_period) / jupiter_orbit_period), 0., 1., 0.);

	glTranslatef(5.20 + 10, 0., 0.);

	glBindTexture(GL_TEXTURE_2D, tex[5]);
	int jupiter_rotation_period = 413.53; 
	glRotatef(3, 1, 0, 0); 
	glRotatef(360 * (float(ms % jupiter_rotation_period) / jupiter_rotation_period), 0., 1., 0.); 
	glShadeModel(GL_SMOOTH);
	SetMaterial(1.0, 1.0, 1.0, 20.0);
	Sphere(2.0, 50, 50); 
	glPopMatrix();
	...
	int saturn_orbit_period = int(o_p(9.58 + 11));
	glRotatef(360 * (float(ms % saturn_orbit_period) / saturn_orbit_period), 0., 1., 0.); 
	...
	glRotatef(-27, 1, 0, 0); 
	int saturn_rotation_period = 444.03; 
	glRotatef(360 * (float(ms % saturn_rotation_period) / saturn_rotation_period), 0, 1, 0); 
	Sphere(1.9, 50, 50);  
	glPopMatrix();
	glDisable(GL_TEXTURE_2D);
	...
	int uranus_orbit_period = int(o_p(19.20 + 12));
	...
	glRotatef(-98, 1, 0, 0); 
	int uranus_rotation_period = 718.33; 
	...
	int neptune_orbit_period = int(o_p(30.05 + 13));
	...
	int neptune_rotation_period = 671.25; 
	...
	int pluto_orbit_period = int(o_p(49.3 + 14));
